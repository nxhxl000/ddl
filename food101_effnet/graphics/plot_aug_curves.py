...
OUT_PATH = Path(__file__).with_name("aug_curves.png")

# собираем только эксперименты с аугментацией (в имени есть "aug_")
run_dirs = sorted(
    d for d in RUNS_DIR.iterdir()
    if d.is_dir() and "aug_" in d.name
)

if not run_dirs:
    raise SystemExit(f"Не нашёл ни одной папки с 'aug_' в {RUNS_DIR}")
...
for run_dir in run_dirs:
    ...
    # пример: "20251125_195259_aug_color_lr3e-4" -> "aug_color_lr3e-4"
...
# сохранить картинку в graphics/aug_curves.png
